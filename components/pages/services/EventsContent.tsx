...
export default function EventsContent({ navigationDict, footerDict, serviceDict, lang }: EventsContentProps) {
  return (
    <main className="min-h-screen bg-terra-white">
           ...
           <div className="text-center mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {serviceDict.services.items.map((item: any, index: number) => {
                const icons = [PartyPopper, Sun, Calendar];
                ...
                return (
                  <motion.div 
                    ...
                    className={`bg-white p-6 rounded-2xl shadow-xl border-t-4 ${color === 'brown' ? 'border-terra-brown' : 'border-terra-gold'} relative group hover:-translate-y-2 transition-transform duration-300`}
                  >
                     <div className="mt-4 mb-4 flex justify-center">
                        <div className={`p-3 ${color === 'brown' ? 'bg-terra-brown/10' : 'bg-terra-gold/10'} rounded-full`}>
                           <IconComponent className={`w-6 h-6 ${color === 'brown' ? 'text-terra-brown' : 'text-terra-gold'}`} />
                        </div>
                     </div>
                     <h4 className={`text-xl font-bold ${color === 'brown' ? 'text-terra-brown' : 'text-terra-brown'} text-center mb-4 uppercase`}>{item.name}</h4>
         ...
         <div className="container mx-auto max-w-6xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
               ...
               >
                  ...
                  <ul className="space-y-4">
                     {serviceDict.approach.steps.map((item: string, i: number) => (
                       <li key={i} className="flex items-center gap-4 text-gray-700 font-medium">
      ...
      <section className="py-24 px-6 bg-terra-brown relative overflow-hidden text-center text-white">
        <div className="container mx-auto max-w-4xl relative z-10">
          ...
          <Link
            href={`/${lang}/contact?subject=events`}
